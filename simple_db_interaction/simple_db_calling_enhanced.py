...
# Main function to integrate Ollama with the single query tool and send results back to the model
async def main():
    client = ollama.AsyncClient()

    # User query
    prompt = "List all products with 'Mouse' in the name."
    print('Prompt:', prompt)
    ...
    system_role = ("Use these column names: product_id,product_name,category,price,stock,description. Use can use "
                   "'like' to filter description only if needed. Send value in lowercase without % sign."
                   )
    ...
    if response.message.tool_calls:
        for tool in response.message.tool_calls:
            ...
            if function_to_call := available_functions.get(tool.function.name):
                ...
                # Send the results back to the model for a natural response
                followup_response: ChatResponse = await client.chat(
                    'qwen2.5-coder:7b',
                    messages=[
                        {'role':'system', 'content':'Rephrase using well formatted bullets in Arabic friendly language'},
                        {'role': 'user', 'content': function_output},
                        # {'role': 'assistant', 'content': function_output},
                    ]
                )
                print('Follow-up response:', followup_response.message.content)
            else:
